...
#include <LiquidCrystal_I2C.h> // libreria di gestione del display lcd addr en,rw,rs,d4,d5,d6,d7,bl,blpol
LiquidCrystal_I2C lcd(0x27, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE); // definisce la tipologia del display

#include <Ethernet.h>
//Settare l`IP manualmente
byte ip[] = {192, 168, 2, 100};   //Inserire il vostro IP
byte gateway[] = {192, 168, 2, 1}; //Inserire il vostro gateway (lasciare invariato)
...
char auth[] = "607fbeb9f279455182868fbbc244c498";
#define W5100_CS  10
#define SDCARD_CS 4
...
int refreshPage = 5; //di default è 10sec. 
...
String buttonText[10] = {
  "01. Postazione 01","02. Postazione 02","03. Postazione 03","04. Postazione 04","05. Postazione 05 ","06. Postazione 06","07. Postazione 07","08. Postazione 08","09. Postazione 09","10. Postazione 10"};
...
void setup() {
  ...
  //Blynk.begin(auth);
  Serial.println("waiting new card!!");
  lcd.print("waiting new card!!");
}
...
void loop() {
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("New card reading!!");
  lcd.setCursor(0, 2);
  lcd.print("UID: ");
  for (byte i = 0; i < mfrc522.uid.size; i++) 
    {
      codiceLetto+= String (mfrc522.uid.uidByte[i],HEX);
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if(reading){
          ...
          //se l`imput è T l`uscita equivale a 0
          if(c == 'T') {
            lcd.setCursor(0, 3);
            lcd.clear();
          }
...
void printHtmlFooter(EthernetClient client){
    ...
    client.println("\n<h3 align=\"center\">&copy; Andrea Fonda - AlfaFox Technologies <br> Italia - 2018 -");
    client.println(rev);
